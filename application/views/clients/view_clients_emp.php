...
<div class="col-sm-offset-4">
<p style="color:green;"><?php print_r($this->session->flashdata('details'));?></p>
</div>
		...
		<tr>
			...
			<td><form name="edit" class="form-horizontal" action="<?php echo base_url('Leads/edit');?>"  method="POST">
					...
					<input type="submit" name="submit" class="btn btn-default btn-sm" value="Services">
						...
						<td>
				<form name="view_client_details" class="form-horizontal" action="<?php echo base_url('Leads/view_client_details');?>"  method="POST">
					<input type="hidden" name="client" value="<?php echo $r->client_id;?>">
					<input type="submit" name="submit" class="btn btn-primary btn-sm" value="View">
				</form>
			</td>
